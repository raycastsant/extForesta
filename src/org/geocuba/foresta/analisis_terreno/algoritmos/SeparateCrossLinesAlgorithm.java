...
public class SeparateCrossLinesAlgorithm
         extends
            GeoAlgorithm {
   ...
   public boolean processAlgorithm() throws GeoAlgorithmExecutionException {
     ...
     // final int iType;
      ...
      m_side = m_Parameters.getParameterValueAsInt(SIDE);

      sFieldNames = new String[1];//m_iPointsInSection * 2 + 2];
      types = new Class[1];
     /* for (i = -m_iPointsInSection, iField = 0; i < m_iPointsInSection + 1; i++, iField++) {
      ...
      sFieldNames[sFieldNames.length-1] = "lado";
   ...
   public void defineCharacteristics() {
      ...
      try {
         ...
         m_Parameters.addNumericalValue(SIDE, Sextante.getText("The_side_of_the_line_to_draw"),
        		 AdditionalInfoNumericalValue.NUMERICAL_VALUE_INTEGER, 0, 0, 2 );
   ...
   private void addPoint(final double x,
    ...
    //  int iField;
     // double x2, y2;
    //  double dElevation;
      final Object value[] = new Object[1];//m_iPointsInSection * 2 + 2];
   ...
   //Dividiendo en dos
      ...
      double midlx = (X1 + X2)/2;
      double midly = (Y1 + Y2)/2;
      
     if(m_side == AlgUtils.DERECHA)
     { 	 
      c[0] = new Coordinate(X1, Y1);
      c[1] = new Coordinate(midlx, midly);
      value[value.length-1] = new String("D");
      m_Result.addFeature(gf.createLineString(c), value);
     }
     else
     if(m_side == AlgUtils.IZQUIERDA)
     {	 
      c[0] = new Coordinate(midlx, midly);
      c[1] = new Coordinate(X2, Y2);
      value[value.length-1] = new String("I");
      m_Result.addFeature(gf.createLineString(c), value);
     }
     else
     if(m_side == AlgUtils.AMBOSLADOS)
     {
    	 c[0] = new Coordinate(X1, Y1);
         c[1] = new Coordinate(midlx, midly);
         value[value.length-1] = new String("D");
